...
BigData BigData::operator/(const BigData & bigData)
{
	...
	if (!IsINT32Overflow() && !bigData.IsINT32Overflow())			//范围内 直接运算
	{
		return BigData(_value / bigData._value);
	}
...
string BigData::Div(string left, string right)   //优化：引用
{
	string sRet;
	...
	for (int iIdx = 0; iIdx < LSize; )
	{
		...
		if (!IsLeftBig(pLeft, Datalen, pRight, right.size() - 1))				//左数小的话，在商的当前位补 0， 并向后移位
		{
			sRet.append(1, '0');
			Datalen++;
...
char BigData::SubLoop(char* pLeft, int LSize, char *pRight, int RSize)  //传参
{
